...
一个可复用的演示文稿工作流 Skill，适合把主题、目标和素材逐步整理成可演示、可修改的 HTML PPT，也可以按需生成 PPTX。它强调先问清楚，再制作；每个关键阶段都要得到确认，避免把一份素材直接粗暴地转换成页面。内置6套视觉风格素材，覆盖从只有Idea到生成演示文件全过程，把控内容提升效率。
...
`需求说明 → 完整 MD 素材库 → 输出格式 → 视觉风格 → 逐页大纲 → 视觉样张 → 分章节制作 → 截图确认 → 局部修改 → 单文件合并`
